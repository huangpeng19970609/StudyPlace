...
- www.baidu.com 主机  localhost:8080 也是主机名，但端口号为80（http）或443（https）就可以省略，比如www.baidu.com: 8080
...
- ```
  从 # 开始的，比如 https://www.baidu.com/#/a/b 那么返回值就是："#/a/b"
  ```

------

1. hash路由模式是这样的：http://xxx.abc.com/#/xx。 带#号，#后面的xx就是hash值的

2. 它不会向服务器发出请求，因此也就不会刷新页面。并且每次hash值发生改变的时候，会触发hashchange事件

3. 如果hash有变化，那么页面将会滚动到hash所连接的位置。
...
4. 请记住 下面几个方法
   ...
   location.href
   location.hash
   使用winodw.hashchange事件来监听url hash的改变
   // location属于window
   ```

5. 如何设置简单的hash路由？

   一种是通过a标签，设置href属性，当点击a标签之后，地址栏会改变，同时会触发hashchange事件。比如如下a链接：
