...
> 冒泡在所有的算法是最简单的，所以我们总是以冒泡算法作为初学
>
> 但运行时间来说，冒泡虽最简单但最差
>
> ⭐ 一个完全与我相反的排序为 (n * n - 1) /  2 这不是指循环次数，而是指循环内的 逻辑执行次数

1. 原理： 比较相邻元素，冒泡相邻元素。一次循环必可将【期值】冒泡至正确位置。
2. 算法复杂度: O(n^2)

```js
function swap(array, a, b) {
  // ES5特别的值之间的交换 =>  相当于 array[a] 与 array[b]的交换
...
> 与冒泡排序一样， 选择排序依旧是双重循环，这导致了二次方的复杂度。
>

1. 原理： 每轮寻找数据结构中的最小值，将其顺序放置。
2. 复杂度 : O(n²)
