...
- 发现自己缓存的文件有 Last Modified 的标识【第一次请求文件的时候，**服务器返回**的一个属性。】

- 第二次请求的时候会添加 If Modified Since
...
>
>    （Etag值会触发缓存，Last-Modified不会触发）
>
> 2. 当修改非常频繁时，Last-Modified会触发缓存，而Etag的值不会触发，重新加载
...
- 宏任务  macrotask
  ...
  3. UI rendering / UI事件、requestAnimationFrame (浏览器)
  4. postMessage / MessageChannel 新的语法！
  5. setImmediate ( Node )，I/O
...
- 强制缓存
  ...
  我们在第一次请求资源时在 http 响应头设置一个过期时间，在时效内都将直接从浏览器进行获取，常见的 http 响应头字段如 Cache-Control 和 Expires
...
- 特性：、
  ...
  1. 只要关闭浏览器， 若弱cookie清空，其浏览器的sessionId会被清空， `但其服务器sessionId绝不会被清空！`
...
>
> ⭐  但混合加密其实已经解决了 窃听、伪造、否认。但不能解决劫持问题。
...
2. 非对称加密：
   ...
   发送密文的一方使用**对方的公开密钥**进行加密处理，对方收到被加密信息后，再使用自己的私有密钥进行解密。
...
1. 服务器的运营人员向第三方机构----数字证书认证机构（Certificate Authority）申请数字证书。
2. CA给与服务器方
   - 申请者公钥
   - 数字签名的密文
   - 证书信息
